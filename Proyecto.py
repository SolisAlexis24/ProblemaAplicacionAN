...
mr = [] #Matriz de resultado

def G1(c): #Despeje 1 de la funcion original
    return -6.81*log(1-(0.06*c))

def G2(c):#Despeje 2 de la funcion original
    return 16.6848*(1-exp(-10*c/68.1))

def ccAS(G):
    dc1 = derivative(G,c0,dx=1e-6) #Se deriva y evalua la funcion
    if(abs(dc1)<1):
        return True #Se comprueba si cumple el criterio
    else:
        return False
